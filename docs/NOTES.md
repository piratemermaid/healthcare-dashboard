# Notes

## Process

1. Create git repo

2. Create vite app

3. Set up basic app stuff (linting, prettier, etc.)
   - For this I use a very simple webapp that I created: https://new-app-guide.vercel.app/ - I made this little site so I can set up new apps quickly.

## Tools

<b>Component library:</b> Material UI

<b>Routing:</b> Tanstack Router

<b>Asynchronous state:</b> Tanstack Query

<b>Patient table with filtering & sorting:</b> MUI X Data Grid - has all functionality needed for quick development

<b>Form library:</b> react-hook-form

## Further Development

Things I would implement if I continued with this project:

- <s>LLM for summaries (& other requirements) - I prioritized core features and stability within the time limit</s> - implemented this for funsies after submitting project
- Nice search bar so user can easily search and not have to click in to datatable filters
- Persist selected table filters in localstorage so user doesn't have to re-select going back to the table page
- Persist user dark mode selection
- DRY python code (like getting patient + error handling)
- Fix form labels getting cut off at top of dialogs
- More/better error handling (including toast messages), form validation, loading handling
- Husky pre-commit linting
